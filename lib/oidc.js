...
module.exports = Class.create({
	...
	oidc_startup: function() {
		...
		var base = (oidc.base_url || this.server.config.get('base_app_url') || '').replace(/\/+$/, '');
		...
		this.web.addURIHandler('/oidc/denied',    'OIDC Denied',    this.handle_oidc_denied.bind(this));
	...
	handle_oidc_bridge: function(args, callback) {
		...
		this.oidc_refresh_and_userinfo(refresh_token, oidc, function(err, userinfo, new_rt) {
			...
			// Check required_roles if configured
			var required_roles = oidc.required_roles || [];
			if (required_roles.length > 0) {
				...
				var has_role = required_roles.some(function(r) { return all_roles.indexOf(r) >= 0; });
				if (!has_role) {
					var denied_user = self.oidc_normalize_username(userinfo);
					self.logDebug(5, "OIDC: access denied for '" + denied_user + "': missing one of required_roles: " + required_roles.join(', '));
						// pixl-server-user appends encodeURIComponent(returnUrl) to the location string,
						// so we add "?r=" as a separator to keep the path "/oidc/denied" intact.
						callback("200 OK", { 'Content-Type': 'application/json' },
							JSON.stringify({ code: 0, location: '/oidc/denied?r=' }));
					return;
				}
			}
	...
	// -------------------------------------------------------------------------
	// /oidc/denied
	// Shown when a user authenticates successfully with Keycloak but lacks
	// the required_roles to access Cronicle. Clears the OIDC cookie so the
	// user can try a different account.
	// -------------------------------------------------------------------------
	handle_oidc_denied: function(args, callback) {
		var html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">' +
			...
			'<p>Please contact your administrator to request access, or ' +
			'<a href="/oidc/bridge?logout=1">sign in with a different account</a>.</p>' +
			'</body></html>';
	...
	oidc_refresh_and_userinfo: function(refresh_token, oidc, done) {
		...
		// pixl-request expects an options object; place the form body in `data`
		req_opts.data = body;

		self.oidcRequest.post(token_url, req_opts, function(err, resp, data) {
